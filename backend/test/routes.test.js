...
describe('Sensor Controller - Ticket 3.1', () => {
  ...
  describe('addSensorReading', () => {
    it('should return 403 if user is not authorized', async () => {
      ...
      const res = makeRes();
      const handler = makeAddSensorReading();
    ...
    it('should create a sensor reading and update bin snapshot', async () => {
      ...
      // Stub SensorReadingFactory.create to return an object with a stubbed save method
      const fakeFactory = { create: sinon.stub().returns({ save: sinon.stub().resolves(readingDoc) }) };
  ...
  const handler = makeAddSensorReading(fakeFactory);
  await handler(req, res);

      expect(findBinStub.calledOnceWith({ _id: binId, userId })).to.be.true;
      expect(fakeFactory.create.calledOnceWithMatch({ userId, binId, fillPct: 85, batteryPct: 60 })).to.be.true;
    ...
    it('should return 500 on DB error', async () => {
      ...
      // Stub SensorReadingFactory.create to throw error on save
      const fakeFactory = { create: sinon.stub().returns({ save: sinon.stub().throws(new Error('DB Error')) }) };
  ...
  describe('getReadingsForBin', () => {
    it('should return 403 if user is not authorized', async () => {
      const req = { user: null, params: { binId: 'b1' } };
      const res = makeRes();
